...
class PacientData {
	...
	public function add(){
		...
		$sql = "insert into ".self::$tablename." (name,lastname,typedoc,cc,gender,day_of_birth,age,gender2,disca,address,address2,address3,address4,phone,phonecel,eps,regimen,extranjero,extranjerostate,email,created_at,tipopoblacion) ";
		$sql .= "value (\"$this->name\",\"$this->lastname\",\"$this->typedoc\",\"$this->numdoc\",\"$this->gender\",\"$this->day_of_birth\",\"$this->age\",\"$this->gender2\",\"$this->disca\",\"$this->address\",\"$this->address2\",\"$this->address3\",\"$this->address4\",\"$this->phone\",\"$this->phonecel\",\"$this->eps\",\"$this->regimen\",\"$this->extranjero\",\"$this->extranjerostate\",\"$this->email\",$this->created_at,$this->tipopoblacion)";
		try{
			Executor::doit($sql);
		}catch(Exception $e){
